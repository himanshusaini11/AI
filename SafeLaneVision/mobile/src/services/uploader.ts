import {API_BASE_URL, DEVICE_ID, DEVICE_SECRET, EVENT_RISK_THRESHOLD, FRAME_UPLOAD_INTERVAL_MS, DEFAULT_GEO, DEFAULT_WEATHER} from '../config';
...
class Uploader {
  ...
  enqueueFrame(summary: PipelineSummary) {
    ...
    const payload: FramePayload = {
      ...
      weather: state.weather ?? DEFAULT_WEATHER,
      meta: {fps: summary.fps, status: summary.status},
    };
  ...
  enqueueEvent(box: HazardBox) {
    if (!API_BASE_URL || box.risk < EVENT_RISK_THRESHOLD) {
      return;
    }
    const now = Date.now();
    const state = usePipelineState.getState();
